package com.kafka.message.produce;
...
public record ProduceRequestV11(Duration maxWait, int minBytes, int maxBytes, byte isolationLevel, int sessionId,
                                int sessionEpoch, List<Topic> topics, List<ForgottenTopicsData> forgottenTopicsData,
                                String rackId) implements RequestBody {

    public static final RequestApi API = RequestApi.of(0, 11);

    public static ProduceRequestV11 deserialize(DataInput input) {
        ...
        return new ProduceRequestV11(maxWait, minBytes, maxBytes, isolationLevel, sessionId, sessionEpoch, topics, forgottenTopicsData, rackId);
    }

    public record Topic(UUID topicId, List<Partition> partitions) {
        ...
        public record Partition(int partition, int currentLeaderEpoch, long fetchOffset, int lastFetchedEpoch,
            ...
            public static Partition deserialize(DataInput input) {
                ...
                return new Partition(partition, currentLeaderEpoch, fetchOffset, lastFetchedEpoch, logStartOffset, partitionMaxBytes);
            }
        }
    }

    public record ForgottenTopicsData(UUID topicId, List<Integer> partitions) {
        ...
        public static ForgottenTopicsData deserialize(DataInput input) {
            ...
            return new ForgottenTopicsData(topicId, partitions);
        }
    }
}
