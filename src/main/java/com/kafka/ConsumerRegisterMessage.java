...
/**
 * Mirrors Go's com.kafka.ConsumerRegisterMessage struct.
...
@NoArgsConstructor
@AllArgsConstructor
public class ConsumerRegisterMessage {
     ...
     */
    public static ConsumerRegisterMessage fromBytes(byte[] data) {
        int port    = ((data[0] & 0xFF) << 8) | (data[1] & 0xFF);
        int topicId = ((data[2] & 0xFF) << 8) | (data[3] & 0xFF);
        return new ConsumerRegisterMessage(port, topicId);
    }
    ...
    public String toString() {
        return "com.kafka.ConsumerRegisterMessage{port=" + port + ", topicId=" + topicId + "}";
    }
}
